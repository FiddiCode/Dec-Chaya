...
if ($(".chain_form_3").length > 0 || $(".form_3").length > 0) {
  if ($("header").css("position") == "fixed") {
    if ($(".outer-page").length > 0) {
      var scrolWidth =
        document.querySelector(".outer-page").offsetWidth -
        document.querySelector(".outer-page").clientWidth;
    } else if ($(".outerpage").length > 0) {
      var scrolWidth =
        document.querySelector(".outerpage").offsetWidth -
        document.querySelector(".outerpage").clientWidth;
    }
    $("header").css(
      "right",
      scrolWidth + -1 * parseInt($(".navbar-top").css("margin-right"))
    );
  }
}
...
function setArrivalDateinForm(form) {
  ...
  var arrivalMonthName = $.datepicker.formatDate(
    "M",
    $(form).find(".arrival-date-input").datepicker("getDate")
  );
...
function setDeptDateinForm(form) {
  ...
  var deptMonthName = $.datepicker.formatDate(
    "M",
    $(form).find(".departure-date-input").datepicker("getDate")
  );
...
function registerClickonGuest() {
  $(".adult-block .no-of-adult").click(function (e) {
    ...
    $(form)
      .find(".totalGuests")
      .html(parseInt(noOfAdult) + parseInt(child.val()));
  });
  $(".children-block .no-of-child").click(function (e) {
    ...
    $(form)
      .find(".totalGuests")
      .html(parseInt(adult.val()) + parseInt(noOfChild));
  });
}
...
function transitionToBottom() {
  ...
  if (window.matchMedia("(max-width: 768px)").matches) {
    if ($(".booking-form-inner").css("display") === "block") {
      ...
      $(form)
        .closest(".bookingform-wrapper")
        .animate({ bottom: "-20%" }, "slow", function () {
...
function init_datePicker(form) {
  ...
  $(form)
    ...
    .datepicker({
      ...
      onSelect: function () {
        ...
        if (
          be_form_variant === "form_2" ||
          be_form_variant === "chain_form_2"
        ) {
          var new_date = $(form)
            .find(".arrival-date-input")
            .datepicker("getDate", "+1d");
  ...
  $(form)
    ...
    .data("datee", checkInDate);
  $(form)
    .find(".departure-date-input")
    .prop("readonly", true)
    .datepicker({
  ...
  $(form)
    ...
    .datepicker(
      "setDate",
      new Date(checkInDate.getTime() + 24 * 60 * 60 * 1000 * lengthOfStay)
    );
}
function sm_update_booking_engine_hotels() {
  var $option = $(".sm_booking_form #booking-engine-groups").find(
    "option:selected"
  );
      ...
      $(this.engines).each(function () {
        if (parseInt(this.propertyId) === parseInt(hotel_id)) {
          ...
          if (be_form_variant === "chain_form_2") {
            if (
              this.bookingEngine === "enquiry" ||
              this.bookingType === "button"
            ) {
              transitionToBottom();
              $(".guest-wrapper, .date-parameter-block, .promo-block").addClass(
                "hidden"
              );
            } else {
              $(
                ".guest-wrapper, .date-parameter-block, .promo-block"
              ).removeClass("hidden");
            ...
            for (i = 1; i <= this.adult; i++) {
              if (i !== adults) {
                ...
                $(f_adults).append(
                  '<div class="no-of-adult selectedNumber">' + i + "</div>"
                );
            ...
            for (i = 0; i <= this.child; i++) {
              if (i !== children) {
                ...
                $(f_child).append(
                  '<div class="no-of-child selectedNumber">' + i + "</div>"
                );
          ...
          var child_title =
            "Age : " + this.childMinAge + "-" + this.childMaxAge + " Years";
...
function init_sm_booking_engine() {
  ...
  var is_chain =
    $("#parent_hotel_id").length && $("#hotel_id").length ? true : false;
  var is_parent =
    is_chain && $("#parent_hotel_id").val() === $("#hotel_id").val()
      ? true
      : false;
  var is_common =
    $("#template_type").length && $("#template_type").val() === "common"
      ? true
      : false;
  var is_same_be = true;
  ...
  $(".sm_booking_form #booking-engine-groups").on("change", function () {
    ...
    var $option = $(".sm_booking_form #booking-engine-hotels").find(
      "option:selected"
    );
    ...
    if (websiteMessagingStatus === "True") {
      formInitSiteMessaging(
        $(".sm_booking_form #booking-engine-hotels")
  ...
  $(".sm_booking_form #booking-engine-hotels").on("change", function () {
    ...
    if (websiteMessagingStatus === "True") {
      formInitSiteMessaging(
        $(this).find("option:selected").attr("data-hotel_id"),
        device,
        is_parent,
        is_chain
      );
  ...
  $(form).each(function () {
    ...
    if (
      (be_form_variant === "form_2" &&
        $(this)
          .closest(".bookingform-wrapper")
          .hasClass("booking-form-container")) ||
      be_form_variant === "chain_form_2"
    ) {
  ...
  if (be_form_variant === "form_2" || be_form_variant === "chain_form_2") {
    ...
    $(document).on(clickEvent, function (e) {
      ...
      if ($(".booking-form-inner").css("display") === "block") {
        ...
        if (
          !$clicked.parents().hasClass("modal-body") &&
          !$clicked.parents().hasClass("abs_parent") &&
          !$clicked.parents().hasClass("ui-datepicker-header") &&
          !$clicked.hasClass("resv_button")
        ) {
          transitionToBottom();
